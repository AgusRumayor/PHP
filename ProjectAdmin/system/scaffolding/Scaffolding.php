...
class Scaffolding {
	...
	function edit()
	{
		if (FALSE === ($id = $this->CI->uri->segment($this->segment2 + 2, 0)))
		{
			return $this->view();
		}
		...
		$data = array(
						...
						'query'		=> $query->row(),
						'action'	=> $this->base_uri.'/update/'.$this->CI->uri->segment($this->segment2 + 2, 0)
					);
	...
	{	
		...
		// Now do the query
		$this->CI->db->update($this->current_table, $_POST, array($primary => $this->CI->uri->segment($this->segment2 + 2, 0)));
	...
	function delete()
	{
		...
		{
			//cambie el segment 4 por el 5
			$message = 'Estas seguro de que quieres borrar: '.$this->CI->uri->segment($this->segment2 + 2, 0);
		}
		else
		{
			$message = $this->lang['scaff_del_confirm'].' '.$this->CI->uri->segment($this->segment2 + 2, 0);
		}
		...
		$data = array(
						...
						'yes'		=> anchor(array($this->base_uri, 'do_delete', $this->CI->uri->segment($this->segment2 + 2, 0)), 'Si' )//( ! isset($this->lang['scaff_yes'])) ? 'si' : $this->lang['scaff_yes'])
					);
	...
	{		
		...
		// Now do the query
		$this->CI->db->where($primary, $this->CI->uri->segment($this->segment2 + 2, 0));
